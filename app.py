...
def _get_or_create_worksheet(sh, worksheet_name: str, header_columns: list[str]):
    try:
        return sh.worksheet(worksheet_name)
    except WorksheetNotFound:
        worksheet = sh.add_worksheet(title=worksheet_name, rows="1000", cols=str(max(20, len(header_columns))))
        worksheet.update([header_columns])
        return worksheet
